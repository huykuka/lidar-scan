"""
Node registry for the HelloWorld application module.
...
which in turn is imported by :func:`~app.modules.discover_modules` at startup.

Side-effects executed at import time:

* :data:`~app.services.nodes.schema.node_schema_registry` receives the
  ``NodeDefinition`` for the ``hello_world`` type.
