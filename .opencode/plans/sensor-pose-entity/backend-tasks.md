...
- [x] **B-01** Create `app/schemas/pose.py`:
...
- [x] **B-02** Add `Pose` to `app/schemas/__init__.py` so it is importable as `from app.schemas import Pose`.

---

## Phase 1 — Database Migration (Data-Only, No DDL)

- [x] **B-03** Update `app/db/models.py — NodeModel.to_dict()`:
...
- [x] **B-04** Update `app/db/migrate.py — ensure_schema()`:
...
- [x] **B-05** Verify migration runs cleanly against existing `config/data.db` (if present). Write a migration smoke test in `tests/modules/` that creates a fake node with flat pose keys in `config_json`, runs `ensure_schema()`, and asserts that `config["pose"]` is populated and flat keys are removed from `config`.

---

## Phase 2 — Repository Layer

- [x] **B-06** Update `app/repositories/node_orm.py — NodeRepository`:
...
- [x] **B-07** Update `app/api/v1/schemas/nodes.py — NodeRecord`:
...
- [x] **B-08** Update `app/api/v1/nodes/service.py — NodeCreateUpdate`:
  - Add `pose: Optional[Pose] = None` field
...
- [x] **B-09** Update `app/modules/lidar/sensor.py — LidarSensor`:
...
- [x] **B-10** Update `app/modules/lidar/registry.py`:
...
- [x] **B-11** Update `app/modules/calibration/calibration_node.py`:
  - Import `Pose` from `app.schemas.pose`
...
- [x] **B-12** Update `app/modules/calibration/history.py`:
...
- [x] **B-13** Update `app/api/v1/calibration/service.py — rollback_calibration()`:
...
- [x] **B-14** Update `app/services/nodes/schema.py — PropertySchema`:
...
- [x] **B-15** Unit tests `tests/schemas/test_pose.py`:
...
- [x] **B-16** Unit tests `tests/modules/test_sensor_pose.py`:
...
- [x] **B-17** Unit tests `tests/modules/test_build_sensor.py`:
...
- [x] **B-18** Integration tests `tests/api/test_nodes_pose.py`:
...
- [x] **B-19** Integration test — DB migration backfill:
...
- [x] **B-20** Integration test `tests/api/test_calibration_rollback_pose.py`:
