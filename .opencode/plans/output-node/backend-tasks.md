...
- [ ] **B1.1** Create `app/modules/flow_control/output/` directory with `__init__.py`
- [ ] **B1.2** Create `app/modules/flow_control/output/registry.py` with `NodeDefinition` registration (see `technical.md §2.2`)
...
- [ ] **B1.3** Update `app/modules/flow_control/registry.py` to import the new sub-module:
  ```python
...
- [ ] **B2.1** Create `app/modules/flow_control/output/node.py` with `OutputNode(ModuleNode)` class
- [ ] **B2.2** Implement `__init__(self, manager, node_id, name, config)`:
...
- [ ] **B2.3** Implement `_extract_metadata(payload: Dict) -> Dict`:
...
- [ ] **B2.4** Implement `async on_input(self, payload)`:
...
- [ ] **B2.5** Implement `emit_status(self) -> NodeStatusUpdate`:
...
- [ ] **B3.1** Create `app/modules/flow_control/output/webhook.py` with `WebhookSender` class
- [ ] **B3.2** Implement `from_config(cls, config) -> Optional[WebhookSender]`:
...
- [ ] **B3.6** Add `httpx` to `requirements.txt` if not already present

---

## Phase 4 — API Endpoints

- [ ] **B4.1** Create `app/api/v1/output/` directory with `__init__.py`, `handler.py`, `service.py`, `dto.py`
- [ ] **B4.2** Define Pydantic models in `dto.py`:
...
- [ ] **B4.3** Implement `GET /api/v1/nodes/{node_id}/webhook` in `handler.py`:
...
- [ ] **B4.4** Implement `PATCH /api/v1/nodes/{node_id}/webhook` in `handler.py`:
...
- [ ] **B4.5** Add `_rebuild_webhook(self, config)` method to `OutputNode`:
  - Re-instantiates `self._webhook = WebhookSender.from_config(config)`
  - Allows live config update without restarting node
- [ ] **B4.6** Register output router in API aggregator (e.g., `app/api/v1/__init__.py` or `app/app.py`)
- [ ] **B4.7** Add Swagger annotations to all endpoints (`summary`, `description`, `response_model`, error `responses`)

---

## Phase 5 — DB Migration

- [ ] **B5.1** Confirm no schema change is needed (webhook config lives in `config_json` blob) — no `migrate.py` change required
- [ ] **B5.2** Verify existing `NodeRepository.update_node_config` correctly updates the `config_json` without clobbering other config fields (use merge, not replace)

---

## Phase 6 — Integration

- [ ] **B6.1** Write unit tests in `tests/modules/test_output_node.py`:
...
- [ ] **B6.2** Write unit tests in `tests/modules/test_webhook_sender.py`:
...
- [ ] **B6.3** Write API integration tests in `tests/api/test_output_webhook.py`:
...
- [ ] **B6.4** Write WebSocket registration test in `tests/services/nodes/test_websocket_registration.py`:
  - `test_output_node_skips_websocket_topic_registration` — assert `_ws_topic is None`
- [ ] **B6.5** Confirm all existing tests pass after module addition (`pytest tests/`)
