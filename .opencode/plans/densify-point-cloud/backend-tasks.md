...
- [x] Implement `__init__(self, enabled, algorithm, density_multiplier, quality_preset, preserve_normals)` with all defaults per `api-spec.md` §4
...
- [x] Use `self.density_multiplier` as the effective multiplier
...
- [x] Add full `NodeDefinition` for `"densify"` per `technical.md` §11
  ...
  - All 5 `PropertySchema` entries (throttle_ms, algorithm, density_multiplier, quality_preset, preserve_normals)
...
- [x] Verify the fail-safe: pass an already-dense cloud (multiplier=1.0) → assert `status=skipped`
