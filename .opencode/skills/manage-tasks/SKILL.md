...
2. **Task Folders**: Instead of a single file, you MUST generate a dedicated folder for each feature, or ignore that if the folder already exists:
   - `mkdir -p .opencode/plans/<feature_name>/`
3. **Task Files**: Inside that folder, you MUST scaffold five files with Markdown task checkboxes (`- [ ]`):
