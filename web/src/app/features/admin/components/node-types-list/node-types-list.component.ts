...
@Component({
  selector: 'app-node-types-list',
  imports: [SynergyComponentsModule],
  templateUrl: './node-types-list.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class NodeTypesListComponent implements OnInit {
  ...
  protected enabledInGroup(types: NodeTypeRecord[]): number {
    return types.filter((t) => t.enabled).length;
  }

  protected disabledInGroup(types: NodeTypeRecord[]): number {
    return types.filter((t) => !t.enabled).length;
  }
}
